function price_calculation(cpus, memory, disk, dartfs, sysadmin) {
  ...
  let total_cost = 0;
  if (cpus) total_cost += cost_per_cpu * cpus;
  if (memory) total_cost += cost_per_gb_memory * memory;
  if (disk) total_cost += cost_per_gb_disk * disk;
  if (dartfs) total_cost += cost_per_tb_dartfs * dartfs;
  if (sysadmin) total_cost += sysadmin_cost;
  return total_cost;
}
...
function calculateAndDisplay() {
  ...
  // Update the invoice link so it carries the form data
  const link = document.getElementById("view-invoice");
  if (link) {
    link.href = "invoice/invoice.html" + window.location.search;
  }
}
